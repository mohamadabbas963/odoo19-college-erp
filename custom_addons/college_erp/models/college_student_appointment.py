...
class CollegeAppointment(models.Model):
    ...
    department_id = fields.Many2one(related='student_id.department_id', string="Department", readonly=True)

    appointment_datetime = fields.Datetime(string="Date & Time", default=fields.Datetime.now, tracking=True)
    purpose = fields.Selection([
        ('academic', 'Academic Advising'),
        ('financial', 'Financial Matters'),
        ('registration', 'Registration'),
    ], string="Purpose", required=True, default='academic')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('college.student.appointment') or 'New'
        ...
        for rec in self:
            ...
            invoice_vals = {
                ...
                'invoice_line_ids': [(0, 0, {
                    ...
                    'quantity': 1,
                    'price_unit': 50.0,  # يمكنك ربطها بـ Fee Type لاحقاً
                })],
            }
