...
class CollegeClassroom(models.Model):
    ...
    assigned_student_ids = fields.One2many(
        "college.student", "current_classroom_id", string="Assigned Students"
    )

    current_student_count = fields.Integer(
        string="Current Students", compute="_compute_student_count", store=True
    )

    available_seats = fields.Integer(
        string="Available Seats", compute="_compute_available_seats"
    )
