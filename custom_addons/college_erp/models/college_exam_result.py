...
class CollegeExamResult(models.Model):
    ...
    department_id = fields.Many2one(related='student_id.department_id', store=True, string="Department")
    exam_date = fields.Date(string='Exam Date', default=fields.Date.context_today, required=True)

    result_line_ids = fields.One2many('college.exam.result.line', 'result_id', string='Exam Scores')

    total_marks = fields.Float(string='Total Marks', compute='_compute_final_result', store=True)
    average_percentage = fields.Float(string='Percentage', compute='_compute_final_result', store=True)
    status = fields.Selection([
        ('pass', 'Passed'),
        ('fail', 'Failed')
    ], string='Final Status', compute='_compute_final_result', store=True)
    ...
    def _compute_final_result(self):
        for rec in self:
            ...
            rec.average_percentage = (rec.total_marks / (count * 100)) * 100 if count > 0 else 0
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', '/') == '/':
                vals['name'] = self.env['ir.sequence'].next_by_code('college.exam.result') or '/'
...
class CollegeExamResultLine(models.Model):
    ...
    def _compute_grade(self):
        for line in self:  # تم تصحيح المتغير هنا من rec إلى line لتجنب تداخل الأسماء
            if line.score >= 90:
                line.grade = 'A+'
            elif line.score >= 80:
                line.grade = 'A'
